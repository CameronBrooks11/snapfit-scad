...
module snapfit(dia = def_dia, lip = def_lip, col_h = def_col_h, col_tol = def_col_tol, slot_ratio = def_slot_ratio,
               thickness = undef)
{
    ...
    difference()
    {
        union()
        {
            ...
            translate([ 0, 0, col_h / 2 ]) cylinder(r = col_r - col_tol, h = col_h, center = true); // Mini column

            if (!is_undef(thickness))
            { // optional snapfit foot
                translate([ 0, 0, col_h - head_h - thickness + (head_h) / 2 ]) cylinder(
                    r1 = col_r - col_tol, r2 = col_r - col_tol + lip, h = head_h, center = true); // snapfit foot
            }
        }

        thick = is_undef(thickness) ? 0 : thickness; // Snapfit foot thickness
        translate([ 0, 0, (col_h + head_h + z_fite) - (slot_ratio * (col_h + head_h)) / 2 ]) cube(
            [ head_r1 / 3, head_r1 * 2 + thick * 2, slot_ratio * (col_h + head_h) ], center = true); // Cut in snap fit
    }
}

module snapfit_neg(dia = def_dia, lip = def_lip, col_h = def_col_h, neg_tol = def_neg_tol, slot_ratio = def_slot_ratio,
                   thickness = undef, flip = false)
{
    ...
    rotate([ rot, 0, 0 ]) translate([ 0, 0, -(head_h + neg_tol) + z_offs ]) union()
    {
        translate([ 0, 0, (head_h + neg_tol) / 2 ]) cylinder(r1 = head_r1 + neg_tol, r2 = head_r2 + neg_tol,
                                                             h = head_h + neg_tol, center = true); // snapfit head
        if (!is_undef(thickness))
        { // optional snapfit foot
...
function calculateRadFromAngle(angle, r, height) = let(norm_angle = angle % 90)(angle < 90)
